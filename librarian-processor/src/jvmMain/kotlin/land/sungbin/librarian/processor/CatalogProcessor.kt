...
import land.sungbin.librarian.processor.impl.ColorGenerator
import land.sungbin.librarian.processor.impl.TypographyGenerator
...
internal class CatalogProcessor(environment: SymbolProcessorEnvironment) : SymbolProcessor {
  ...
  private val color by lazy { ColorGenerator(logger, codeGenerator) }
    ...
    resolver.getSymbolsWithAnnotation(catalogCn.canonicalName)
      ...
      .forEach { (type, elements) ->
        ...
        when (type) {
          ...
          KSCatalogs.Color.simpleName -> color.generate(elements as List<KSClassDeclaration>)
        }
      }
